...
class ResultadoCompresion:
    ...
    grafo_mermaid: Optional[str] = None  # <--- NUEVO CAMPO PARA EL ÁRBOL
    es_stub: bool = True
...
class CodificadorHuffman:
    ...
    def _generar_mermaid(self, nodo: Optional[NodoHuffman], total_simbolos: int) -> str:
        """Genera el árbol en formato horizontal (graph RL) para igualar la imagen."""
        if not nodo:
            return ""
        
        # graph RL dibuja desde Derecha (Root) a Izquierda (Hojas)
        lineas = [
            "graph RL",
            "    classDef leaf fill:#fff,stroke:#000,stroke-width:2px,color:#000;",
            "    classDef internal fill:#fff,stroke:#000,stroke-width:2px,color:#000;"
        ]
        contador = [0]
        ...
        def recorrer(n: NodoHuffman) -> str:
            ...
            if n.simbolo is not None:
                ...
                # Formato como en la imagen: "a1: 0.40"
                etiqueta = f"{simbolo_limpio}: {prob:.2g}"
                lineas.append(f"    {my_id}[\"{etiqueta}\"]:::leaf")
            else:
                lineas.append(f"    {my_id}((\"{prob:.2g}\")):::internal")

            if n.izquierda:
                izq_id = recorrer(n.izquierda)
                # Para graph RL, Padre(Der) --> Hijo(Izq)
                lineas.append(f"    {my_id} -- \"0\" --> {izq_id}")
            if n.derecha:
                der_id = recorrer(n.derecha)
                lineas.append(f"    {my_id} -- \"1\" --> {der_id}")
    ...
    def comprimir(self) -> ResultadoCompresion:
        ...
        # Generar el grafo limitando la cantidad para no colgar el navegador
        grafo_mermaid = None
        if len(freq) <= 60:
            grafo_mermaid = self._generar_mermaid(raiz, len(texto))
...
def render_arbol_huffman(res: ResultadoCompresion) -> None:
    if res.nombre_algoritmo == "Huffman" and getattr(res, "grafo_mermaid", None):
        section_label("🌿", "DIAGRAMA DEL ÁRBOL DE HUFFMAN")
        info_box("💡", "Gráfico generado dinámicamente con las probabilidades correspondientes. Puedes hacer hover y expandir la imagen en Streamlit.")
