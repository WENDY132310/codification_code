...
def render_header() -> None:
    st.markdown('<div class="app-header-wrap"><div class="app-header"><div class="header-left"><div class="header-icon">⚡</div><div><p class="app-title">DataLab · Compresión y Tx</p><p class="app-subtitle">Fuente (Huffman) · Canal (FEC) · Modulación (BPSK)</p></div></div></div></div>', unsafe_allow_html=True)
...
def render_codigo_y_formulas(algo: str, stats: EstadisticasInfo) -> None:
    ...
    st.code(_CODE_MAP.get(algo, "# Código no disponible."), language="python")

def render_resultado_compresion(res: ResultadoCompresion) -> None:
    section_label("📦", f"RESULTADO DE COMPRESIÓN (FUENTE) — {res.nombre_algoritmo}")
    c1, c2, c3, c4 = st.columns(4)
...
def render_pasos(pasos: List[Dict[str, Any]]) -> None:
    section_label("🔍", "PROCEDIMIENTO DE FUENTE")
...
# ═════════════════════════════════════════════════════════════════════════════
#  NUEVO: INTEGRACIÓN CANAL Y MODULACIÓN
# ═════════════════════════════════════════════════════════════════════════════

def generar_descarga_b64(datos: bytes, nombre_archivo: str) -> str:
    """Genera un link de descarga nativo HTML para evitar re-runs de Streamlit."""
    b64 = base64.b64encode(datos).decode()
    href = f'<a href="data:application/octet-stream;base64,{b64}" download="{nombre_archivo}" class="dl-btn">⬇️ Descargar Señal Modulada (.bin)</a>'
    return href

def render_sistema_integrado(res_fuente: ResultadoCompresion, tipo_dato: str) -> None:
    st.markdown("<br>", unsafe_allow_html=True)
    section_label("📡", "LABORATORIO INTEGRADO: CANAL Y MODULACIÓN (TX/RX)")
    info_box("🌐", "Se aplicará <b>Codificación de Canal (FEC)</b> para proteger los datos comprimidos, seguido de <b>Modulación Digital</b> para su transmisión.", "violet")

    c1, c2, c3 = st.columns(3)
    tasa_fec = c1.selectbox("Tasa de Codificación (FEC)", ["1/2", "2/3", "4/5", "7/8"], key=f"fec_{tipo_dato}", help="Agrega redundancia para corregir errores.")
    mod = c2.selectbox("Esquema de Modulación", ["BPSK"], key=f"mod_{tipo_dato}", help="Phase-Shift Keying.")
    ber = c3.slider("Ruido de Canal (AWGN)", 0.0, 0.1, 0.02, step=0.01, format="%.3f", key=f"ber_{tipo_dato}", help="Probabilidad de Bit Erróneo (BER).")

    if st.button("🚀 Transmitir y Recuperar", key=f"tx_btn_{tipo_dato}"):
        
        # --- ETAPA 1: CODIFICADOR DE CANAL (TX) ---
        k, n = map(int, tasa_fec.split('/'))
        tamaño_tx_estimado = math.ceil(res_fuente.tamaño_comprimido * (n / k))
        payload_simulado = bytes([random.randint(0, 255) for _ in range(min(5000, tamaño_tx_estimado))]) # Dummy de descarga
        
        st.markdown(f"**1. Codificador de Canal:** Redundancia agregada. Tamaño final de trama: `{fmt_bytes(tamaño_tx_estimado)}`")
        st.markdown(f"**2. Modulador ({mod}):** Trama mapeada a símbolos en banda base.")
        
        # Botón HTML Puro para descargar sin Bug
        st.markdown(generar_descarga_b64(payload_simulado, f"tx_modulada_{tipo_dato}.bin"), unsafe_allow_html=True)
        
        st.markdown("---")
        
        # --- ETAPA 2: DEMODULACIÓN Y DECODIFICACIÓN (RX) ---
        c_rx1, c_rx2 = st.columns([1, 2], gap="large")
        
        with c_rx1:
            st.markdown(f"**Diagrama de Constelación ({mod} Rx)**")
            st.markdown(MapeadorCanalModulacion.simular_constelacion_bpsk(ber), unsafe_allow_html=True)
            st.caption(f"Puntos dispersos por ruido AWGN (BER={ber:.3f})")
            
        with c_rx2:
            st.markdown("**Control y Corrección de Errores (FEC)**")
            log_errores = MapeadorCanalModulacion.simular_fec_interfaz(tasa_fec)
            df_err = pd.DataFrame(log_errores)
            st.dataframe(df_err, use_container_width=True, hide_index=True)
            
            if ber > 0:
                info_box("✅", "El decodificador de canal detectó bits alterados por el ruido y **recuperó** la trama original mediante el cálculo de síndromes.", "green")
            else:
                info_box("✅", "Canal ideal. No se detectaron errores en la demodulación.", "green")

        # --- ETAPA 3: DECODIFICACIÓN DE FUENTE FINAL ---
...
def tab_texto() -> None:
    ...
    section_label("⚙️", "ALGORITMO DE COMPRESIÓN DE FUENTE")
    c_sel, c_btn = st.columns([3, 1])
    with c_sel: algo = st.selectbox("Algoritmo", ["Huffman", "LZW", "RLE"], key="algo_texto", label_visibility="collapsed")
    with c_btn: btn_comp = st.button("▶ Comprimir Fuente", key="btn_texto")
    ...
    if btn_comp:
        ...
        # Pipeline Integrado (Tx/Rx)
        render_sistema_integrado(res, "texto") 
...
def tab_imagen() -> None:
    ...
    with c_btn: comprimir = st.button("▶ Comprimir Fuente", key="btn_img")
    ...
    if comprimir:
        ...
        # Pipeline Integrado (Tx/Rx)
        render_sistema_integrado(res, "imagen")
...
def tab_audio() -> None:
    ...
    with c_btn: btn_comp = st.button("▶ Comprimir Fuente", key="btn_audio")
    ...
    if btn_comp:
        ...
        # Pipeline Integrado (Tx/Rx)
        render_sistema_integrado(res, "audio")
...
def tab_video() -> None:
    ...
    with c_btn:
        btn_comp = st.button("▶ Simular Compresión Fuente", key="btn_video")
    ...
    if btn_comp:
        ...
        # Pipeline Integrado (Tx/Rx)
        render_sistema_integrado(res, "video")
